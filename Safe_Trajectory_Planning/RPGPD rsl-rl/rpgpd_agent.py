...
def build_train_cfg(cfg: RslRPGPDConfig, experiment_name: str, run_name: str) -> Dict[str, Any]:
    return {
        "seed": int(cfg.seed),
...
class SafetyGymVecEnv:
    ...
    def step(self, actions: torch.Tensor):
        ...
        for i in range(self.num_envs):
            ...
            # Training uses unshaped reward (reward/cost separation).
...
class RPGPDTrainerHook:
    def __init__(
        ...
        self.lam = torch.full_like(self.cost_limits, float(cfg.lambda_init))
    ...
    def _estimate_discounted_cost(self) -> torch.Tensor:
    ...
    def update(self) -> Dict[str, float]:
        ...
        for _ in range(int(cfg.num_learning_epochs)):
            ...
            for start in range(0, B, mini_batch_size):
                ...
                adv_l_mb = adv_r_mb - (adv_c_mb * self.lam.view(1, -1)).sum(dim=-1)
                surr_l = torch.min(ratio * adv_l_mb, ratio_clipped * adv_l_mb).mean()

                entropy = 0.0
                if hasattr(self.alg.actor, "output_entropy"):
                    entropy = self.alg.actor.output_entropy.mean()

                loss_pi = -surr_l - float(cfg.entropy_coef) * entropy
                ...
                total_surrogate_l += float(surr_l.item())
        ...
        jc = self._estimate_discounted_cost()
        viol =  (jc - self.cost_limits)
        lam_next = (1.0 - float(cfg.dual_lr) * float(cfg.dual_tau)) * self.lam + float(cfg.dual_lr) * viol
        self.lam = torch.clamp(lam_next, min=0.0, max=float(cfg.lambda_max))

        denom = max(1, updates)
        out: Dict[str, float] = {
            "surrogate_lagrangian": float(total_surrogate_l / denom),
            "policy": float(total_loss_pi / denom),
            "value": float(total_value / denom),
            "value_reward": float(total_value_r / denom),
            "value_cost": float(total_value_c / denom),
            "entropy": float(total_entropy / denom),
            "kl_distance": float(total_kl / denom),
        }
        for i in range(int(self.cost_limits.numel())):
            out[f"dual_lambda_{i}"] = float(self.lam[i].item())
            out[f"constraint_violation_{i}"] = float(viol[i].item())
            out[f"discounted_cost_{i}"] = float(self.last_disc_cost[i].item())
