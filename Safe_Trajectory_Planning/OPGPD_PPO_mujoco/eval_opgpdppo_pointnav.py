"""
eval_opgpdppo_pointnav.py

Evaluate a saved checkpoint on Safety-Gymnasium point navigation for OPG-PD PPO.

Usage:
  python eval_opgpdppo_pointnav.py --config config_pointnav_OPGPD.json --checkpoint runs_pointnav_opgpd/ckpt_epoch_001.pt
  python eval_opgpdppo_pointnav.py --config config_pointnav_OPGPD.json --checkpoint runs_pointnav_opgpd/ckpt_epoch_001.pt --num_episodes 10 --parallel 4
"""
...
from opgpd_ppo_agent import OPGPDPPOAgent, OPGPDPPOConfig
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--policy", type=str, default="hat", choices=["hat", "pred"], help="Evaluate policy: hat or pred")
...
def build_env_and_agent(
...
) -> Tuple[RewardCostWrapper, OPGPDPPOAgent]:
    ...
    if "lambda_hat" in ckpt:
        agent.lambda_hat = np.asarray(ckpt["lambda_hat"], dtype=np.float32)
    if "lambda_pred" in ckpt:
        agent.lambda_pred = np.asarray(ckpt["lambda_pred"], dtype=np.float32)
    agent.pi_hat.eval()
    agent.pi_pred.eval()
    agent.v_r.eval()
    agent.v_c.eval()
    agent.set_active_policy(policy)
...
def eval_worker(args: Tuple[str, str, int, int, str, str]) -> Dict[str, Any]:
    config_path, checkpoint_path, seed, max_steps, device, policy = args
    torch.set_num_threads(1)
    cfg = load_config(config_path)
    env, agent = build_env_and_agent(cfg, checkpoint_path, device, policy)
...
def main():
    ...
    if args.parallel > 1:
        ...
        with ctx.Pool(processes=int(args.parallel)) as pool:
            summaries = pool.map(
                eval_worker,
                [(args.config, args.checkpoint, s, max_steps, device, args.policy) for s in seeds],
            )
    else:
        summaries = [eval_worker((args.config, args.checkpoint, s, max_steps, device, args.policy)) for s in seeds]
    ...
    print(
        f"[Eval] policy={args.policy} episodes={len(summaries)} return_mean={ep_rets.mean():.2f} "
    ...
    if args.save_gif or args.save_mp4 or args.save_mp4_all or args.save_mp4_each:
        ...
        env, agent = build_env_and_agent(cfg, args.checkpoint, device, args.policy, render_mode="rgb_array", env_kwargs=env_kwargs)
