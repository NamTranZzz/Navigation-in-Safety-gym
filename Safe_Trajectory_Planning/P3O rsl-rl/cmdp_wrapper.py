...
class CMDPConfig:
    ...
    # Reward shaping term: r_total = reward_scale * r_env + dot(cost_coeffs, costs)
    # Use negative coefficients to penalize costs.
    cost_coeffs: Tuple[float, ...] = (0.0,)
...
class RewardCostWrapper:
    ...
    def __init__(
        ...
        if len(self.cfg.cost_limits) != len(self.cfg.cost_coeffs):
            raise ValueError(
                f"cost_limits length ({len(self.cfg.cost_limits)}) must match cost_coeffs ({len(self.cfg.cost_coeffs)})"
            )
    ...
    def step(self, action: np.ndarray):
        ...
        coeff = np.asarray(self.cfg.cost_coeffs, dtype=np.float32)
        shaped_r = float(r + float(np.dot(coeff, costs)))

        info["reward"] = shaped_r
        info["reward_unshaped"] = float(r)
        info["reward_shaped"] = float(shaped_r)
        info["costs"] = costs
        info["cost_limits"] = np.asarray(self.cfg.cost_limits, dtype=np.float32)
        return obs, shaped_r, bool(terminated), bool(truncated), info
