...
Runnable entrypoint:
- Builds Nav2DEnv (dynamics-only)
- Wraps it with RewardCostWrapper (reward + multi-cost CMDP)
- Trains P3O with fixed kappa=20 (paper default)

Usage:
  python train_p3o_nav2d.py --config config_nav_p3o.json --epochs 50 --device cpu

Notes:
- The provided config matches the paper's Navigation defaults (kappa=20, gamma=0.99, lambda=0.97, clip=0.2).
- For quick sanity checks, override steps_per_epoch/epochs via CLI.
"""
...
from eval_p3o_nav2d import rollout_episode, animate_trajectory
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--save_dir", type=str, default="runs_nav2d", help="Directory to save logs/checkpoints")
...
def _build_eval_env(cfg: Dict[str, Any]) -> RewardCostWrapper:
...
def _rollout_summary(env: RewardCostWrapper, agent: P3OAgent, max_steps: int, seed: int) -> Dict[str, Any]:
    ...
    return {
        ...
        "EpLen": ep_len,
        "Terminated": bool(ep_len > 0 and float(np.linalg.norm(info["goal_pos"] - info["agent_pos"])) <= float(info["goal_radius"])),
    }
...
def main():
    ...
    # Build env
    env_cfg = Nav2DConfig(**cfg["env"])
    env = Nav2DEnv(env_cfg, render_mode=("human" if args.live else None))

    # Wrap with reward/cost
    cmdp_cfg = CMDPConfig(
        cost_limits=tuple(cfg["cmdp"]["cost_limits"]),
        distance_threshold=float(cfg["cmdp"].get("distance_threshold", 1.0)),
        reward_scale=float(cfg["cmdp"].get("reward_scale", 1.0)),
    ...
    # Build agent + trainer
    agent = P3OAgent(obs_dim=wrapped.obs_dim(), act_dim=wrapped.act_dim(), num_costs=len(cmdp_cfg.cost_limits), cfg=algo_cfg, device=args.device)
    trainer = P3OTrainer(env=wrapped, agent=agent, cost_limits=np.asarray(cmdp_cfg.cost_limits, dtype=np.float32), cfg=algo_cfg)

    eval_max_steps = int(args.eval_max_steps) if args.eval_max_steps is not None else int(env_cfg.max_steps)

    # Save a copy of resolved config
    resolved = {
        "env": cfg["env"],
        "cmdp": cfg["cmdp"],
        "algo": {
    ...
    print(f"[Nav2D] obs_dim={wrapped.obs_dim()} act_dim={wrapped.act_dim()} costs={len(cmdp_cfg.cost_limits)}")
    print(f"[Train] epochs={epochs} steps_per_epoch={algo_cfg.steps_per_epoch} device={args.device} seed={seed}")
    print(f"[Defaults] kappa=20, gamma=0.99, lam=0.97, clip=0.2 (paper Navigation defaults)")
    ...
    if args.eval_episodes > 0:
        ...
        try:
            ...
            eval_agent.v_r.eval()
            eval_agent.v_c.eval()
            traj, _ = rollout_episode(eval_env, eval_agent, max_steps=eval_max_steps, seed=seed + 999999, deterministic=True)
            if last_ckpt_path is not None:
                mp4_name = f"eval_rollout_{last_ckpt_path.stem}.mp4"
            else:
                mp4_name = "eval_rollout_final.mp4"
            mp4_path = save_dir / mp4_name
            animate_trajectory(traj, save_path=str(mp4_path), fps=30)
