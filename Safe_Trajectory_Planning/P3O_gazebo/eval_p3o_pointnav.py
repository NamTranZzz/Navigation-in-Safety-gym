"""
eval_p3o_pointnav.py

Evaluate a saved checkpoint on a Gazebo point navigation env (Gymnasium wrapper).

Usage:
  python eval_p3o_pointnav.py --config config_pointnav_p3o.json --checkpoint runs_pointnav/ckpt_epoch_001.pt
...
from gazebo_env import GazeboPointNavEnv, GazeboPointNavConfig
...
def build_env_and_agent(cfg: Dict[str, Any], checkpoint_path: str, device: str, render_mode: str | None = None) -> Tuple[RewardCostWrapper, P3OAgent]:
    env_cfg = GazeboPointNavConfig(**cfg["env"])
    env = GazeboPointNavEnv(env_cfg, render_mode=render_mode)
...
@torch.no_grad()
def rollout_frames(env: RewardCostWrapper, agent: P3OAgent, max_steps: int, seed: int | None, fps: int):
...
def save_frames(frames: List[np.ndarray], save_path: Path, fps: int) -> None:
...
def main():
    ...
    if args.save_gif or args.save_mp4 or args.save_mp4_all:
        env, agent = build_env_and_agent(cfg, args.checkpoint, device, render_mode="rgb_array")
        if args.save_gif or args.save_mp4:
            frames = rollout_frames(env, agent, max_steps=max_steps, seed=args.seed, fps=args.fps)
            if args.save_gif:
                out = save_dir / "eval_rollout.gif"
                save_frames(frames, out, fps=args.fps)
                print(f"  [Saved] {out}")
            if args.save_mp4:
        ...
        if args.save_mp4_all:
            all_frames = []
            for s in seeds:
                frames = rollout_frames(env, agent, max_steps=max_steps, seed=s, fps=args.fps)
                all_frames.append(frames)
            merged = _concat_frames(all_frames, pad_frames=10)
            out = save_dir / "eval_rollout_all.mp4"
            save_frames(merged, out, fps=args.fps)
            print(f"  [Saved] {out}")
